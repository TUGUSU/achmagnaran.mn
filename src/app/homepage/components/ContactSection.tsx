...
function validate(f: FormState): string | null {
  ...
  if (f.phone.trim().replace(/\D/g, '').length < 8)
...
export default function ContactSection() {
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      label: 'Хаяг',
      value: 'Сонгино Хайрхан дүүрэг, 11-р хороо, АОС 3-32 тоот, Улаанбаатар',
      href: null,
    },
  ];
  ...
  return (
    <section id="contact" ref={sectionRef} className="py-24 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2/3 h-64 opacity-10 pointer-events-none"
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-7">
              ...
              <div className="flex flex-col gap-3">
                {contactInfo.map((info) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-[10px] text-cream-subtle uppercase tracking-widest font-semibold mb-1">{info.label}</div>
                      {info.href ? (
            ...
            <div className="scroll-reveal" style={{ transitionDelay: '0.1s' }}>
              <h3 className="font-display font-semibold text-cream text-base mb-4">Нийгмийн сүлжээ</h3>
            ...
            {/* Company card */}
            <div
              className="p-5 rounded-sm border scroll-reveal"
              style={{ borderColor: 'rgba(212,168,83,0.22)', backgroundColor: 'rgba(212,168,83,0.04)', transitionDelay: '0.18s' }}
            >
              <div className="text-[10px] text-accent font-semibold tracking-[0.2em] uppercase mb-3">Компанийн мэдээлэл</div>
              {[
                { k: 'Нэр',        v: 'Ачмаг Наран ХХК' },
                { k: 'Байгуулагдсан', v: '2014 он' },
                { k: 'Чиглэл',    v: 'Мах, махан бүтээгдэхүүн үйлдвэрлэл' },
              ].map((r) => (
                <div key={r.k} className="flex items-start gap-2 mb-1.5 last:mb-0">
                  <span className="text-[12px] text-cream-subtle w-28 flex-shrink-0">{r.k}:</span>
                  <span className="text-[13px] text-cream">{r.v}</span>
                </div>
              ))}
            </div>
          </div>
